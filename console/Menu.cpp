...
void Menu::display()
{
  ...
  if (this->screen == RECOVERY_OPTIONS)
  {
	...
	if (this->screen == LIST_MENU)
	{
		...
		for (int x = start; x < start + PAGE_SIZE && x < (int)get->getPackages().size(); x++)
		{
	...
	{
		if (this->position < 0 || this->position > (int)get->getPackages().size())
		{
...
void Menu::moveCursor(int diff)
{
	...
	else if (position >= (int)get->getPackages().size())
	{
