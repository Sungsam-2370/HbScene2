...
AppDetailsContent::AppDetailsContent(Package *package, bool useBannerIcons)
	...
	, details(i18n("contents.placeholder1"), 20 / SCALER, &HBAS::ThemeManager::textDescription, false, PANE_WIDTH + 20 / SCALER)
	, changelog(i18n("contents.placeholder2"), 20 / SCALER, &HBAS::ThemeManager::textDescription, false, PANE_WIDTH + 20 / SCALER)
	...
	, viewSSButton(i18n("contents.readmore"), Y_BUTTON, false, 15)
{
	...
	changelog.position((MARGIN / SCALER + 30), viewSSButton.y + viewSSButton.height + 30);
	changelog.hidden = true;
	super::append(&changelog);

	if (useBannerIcons) {
		screenshotsContainer.position(BANNER_X / SCALER - 40, viewSSButton.y + viewSSButton.height + 15);
