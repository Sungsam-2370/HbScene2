...
void Sidebar::addHints()
{
	...
	const int bottomY = SCREEN_HEIGHT - 30/SCALER;
	const int iconX   = 10/SCALER;
	hider->position(iconX, bottomY);
	hint->position(iconX + hider->width + 5/SCALER, bottomY + 1);
...
void Sidebar::render(Element* parent)
{
  ...
  return;
	...
	CST_Rect clipRect = { 0, catAreaTop, width, SCREEN_HEIGHT - catAreaTop };
...
void Sidebar::updateCategoryScroll()
{
	...
	const int visibleEnd   = SCREEN_HEIGHT - 30/SCALER;
