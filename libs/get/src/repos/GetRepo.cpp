...
std::vector<std::unique_ptr<Package>> GetRepo::loadPackages()
{
	...
	for (int i = 0; i < (int)total; i++)
	{
		...
		if (cur.HasMember("zip_url"))
			package->zipUrl = cur["zip_url"].GetString();
		if (cur.HasMember("zip_url_2"))
			package->zipUrl2 = cur["zip_url_2"].GetString();
		if (cur.HasMember("zip_url_3"))
			package->zipUrl3 = cur["zip_url_3"].GetString();
		if (cur.HasMember("zip_url_4"))
			package->zipUrl4 = cur["zip_url_4"].GetString();

		// ruta del .nsp a instalar automaticamente tras la extraccion; si no
		// existe esta clave, install_nsp queda vacio y no se instala nada
...
{
	// If the package has a direct zip URL, use it
	if (!package.zipUrl.empty())
		return package.zipUrl;
